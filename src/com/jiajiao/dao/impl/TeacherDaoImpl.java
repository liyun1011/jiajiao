...
public class TeacherDaoImpl implements TeacherDao {
	...
	public Teacher findByPhoneAndPwd(String phone, String pwd) {
		...
		try {
			...
			//接单次数
			String querySql1 = "SELECT COUNT(b.oId) jiedanCount\n" +
					"FROM `t_teacher` a \n" +
					"LEFT JOIN `t_takeorders` b ON a.teacherId = b.teacherId\n" +
					"WHERE takeStatus = 4 AND a.`teacherId` = " + teacher.getTeacherId() ;
			Integer jiedanCount = jdbcTemplate.queryForObject(querySql1, Integer.class);
			//申请次数
			String querySql2 = "SELECT COUNT(oId) applyCount\n" +
					"FROM `t_teacher` a \n" +
					"LEFT JOIN `t_orders` b ON a.teacherId = b.publisher2\n" +
					"WHERE orderStatus = 23 AND a.`teacherId` = " + teacher.getTeacherId() ;
			Integer applyCount = jdbcTemplate.queryForObject(querySql2, Integer.class);
			//预约次数
			String querySql3 = "SELECT COUNT(oId) bookingCount\n" +
					"FROM `t_teacher` a \n" +
					"LEFT JOIN `t_orders` b ON a.teacherId = b.publisher2\n" +
					"WHERE orderStatus = 24 AND a.`teacherId` = " + teacher.getTeacherId() ;
			Integer bookingCount = jdbcTemplate.queryForObject(querySql3, Integer.class);

			teacher.setJiedanCount(jiedanCount);
			teacher.setApplyCount(applyCount);
			teacher.setBookingCount(bookingCount);
			return teacher;
		} catch (Exception e) {
			return null;
		}
	...
	public List<Teacher> findAllTeacherList() {
		...
		String sql = "SELECT t_teacher.teacherId,teacherName,phone,wxNumber,gender, (CASE areaId WHEN 1 THEN '黄埔区'	WHEN 2 THEN '天河区' WHEN 3 THEN '番禺区' WHEN 4 THEN '越秀区' WHEN 5 THEN '海珠区'	WHEN 6 THEN '白云区'  WHEN 7 THEN '花都区'  WHEN 8 THEN '荔湾区'  WHEN 9 THEN '南沙区'  WHEN 10 THEN '从化区'  WHEN 11 THEN '增城区' END) as area,address,identity,t_dictinfo.content as identityName,IFNULL(a.num1,0)+IFNULL(b.num2,0)+IFNULL(c.num3,0) as teachingNum,lastLoginTime,isUse,isCheck "
				+ " FROM t_teacher "
